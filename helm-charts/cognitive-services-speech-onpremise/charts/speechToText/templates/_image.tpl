...
{{- $_ := set . "millicores" (mul .numberOfConcurrentRequest 1150) }}
...
{{- end }}
{{- $cores := div $_.millicores 1000 }}
{{- $_ := set . "cores" $cores }}
{{- if mod $_.millicores 1000 }}
    {{- $_ := set . "cores" (add1 $cores) }}
{{- end }}
{{- $coreslimit := mul $_.cores 2 }}
{{- $_ := set . "coresLimit" $coreslimit }}
{{- end -}}
